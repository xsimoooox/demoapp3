...
var pendingZoomTarget = new THREE.Vector3();

document.addEventListener('mousedown', onDocumentMouseDown, false);

function onDocumentMouseDown(event) {
  // Allow interaction with the security overlay
  if (event.target.closest('#security-overlay')) return;
  ...
  if (intersects.length > 0) {
    // Instead of immediate zoom, show security overlay
    document.getElementById('security-overlay').classList.remove('hidden');
    pendingZoomTarget.copy(intersects[0].point).normalize().multiplyScalar(0.52);
    pendingZoom = true;
  }
}
...
document.getElementById('access-btn').addEventListener('click', function() {
  if (pendingZoom) {
    ...
    isZooming = true;
    zoomTarget.copy(pendingZoomTarget);
    controls.enabled = false; // Disable interaction during zoom
  } else {
    // Fallback if somehow pendingZoom is false
    window.location.href = 'map/index.html';
  }
});
